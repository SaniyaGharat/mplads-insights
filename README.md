...
2. **Hardware & GNN Nondeterminism**: Despite setting fixed random seeds across PyTorch and NumPy, certain sparse graph convolution operations and CUDA/CPU atomic reductions can exhibit slight floating-point score variations across different hardware architectures. Exact loss/score values may shift slightly if `ml/build_pyg_graph.py` is re-run and regenerates the underlying graph object, even with fixed random seeds; run-to-run stability against a FIXED graph object is confirmed (see repo history), but absolute values are not guaranteed stable across independent full pipeline reruns.
